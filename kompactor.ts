    ...
    try {
      ...
      for (const host of this.hosts) {
        ...
        for (const [hourKey, filesMap] of allFiles) {
          ...
          const basePathIndex = parts.findIndex(p => p === 'home-0');
          const basePath = parts.slice(0, basePathIndex + 1).join('/');
            ...
            for (const snapshotFile of jsonFiles) {
              ...
              for (const [dbId, dbInfo] of metadata.databases) {
                for (const [tableId, tableFiles] of dbInfo.tables) {
                  const hasFilesFromHour = tableFiles.some(f => existingFiles.includes(join(this.dataDir, f.path)));
                  if (hasFilesFromHour) {
